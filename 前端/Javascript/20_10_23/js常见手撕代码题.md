...
Function.prototype.myCall = function (ctx) {
  ...
  let args = [];
  for (let i = 1; i < len; i++) args.push(`arguments[${i}]`);
...
Function.prototype.myApply = function (ctx, args) {
  ...
  let res = eval(`ctx.fn(${_args.length ? _args : ""})`);
  delete ctx.fn;
  return res;
};
...
Function.prototype.myBind = function (ctx) {
  ...
  function F() {
    ...
    return _self.apply(this instanceof F ? this : ctx, [...args1, ...args2]);
  }
  function T() {}
  T.prototype = _self.prototype;
  F.prototype = new T();
  return F;
};
...
function deepClone(obj, map = new WeakMap()) {
  let isComplexType = deepClone.isComplexType;
  let types = deepClone.complexType;

  if (map.has(obj)) return map.get(obj);
  if (types.has(obj.constructor)) return new obj.constructor(obj);
  ...
  for (let key of Reflect.ownKeys(obj)) {
    let val = obj[key];
    if (isComplexType(val) && typeof val !== "function")
      cloneObj[key] = deepClone(val, map);
  }
  return cloneObj;
}

deepClone.isComplexType = (obj) =>
  (typeof obj === "object" || typeof obj === "function") && obj !== null;
deepClone.complexType = new Set([
  Array,
  Date,
  Map,
  Set,
